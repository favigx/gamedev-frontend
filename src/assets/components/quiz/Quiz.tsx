...
function Quiz({ roomId }: { roomId: string }) {
    ...
    return (
        <>
            {jwtDecode(localStorage.getItem("token") || "").sub == selectedRoom.createdBy && !currentQuestion ? (
                <div>
                ...
                <div>
                    {currentQuestion ? (
                        <div>
                            <h3 id="questionTitle">Fråga #{currentQuestionIndex + 1}</h3>
                            <p>
                                <strong>{currentQuestion.question}</strong>
                            </p>
                            <div>
                                {currentQuestion.answers.map((answer, index) => (
                                    <button key={index} onClick={() => handleAnswerClick(currentQuestion.questionId, answer)} disabled={hasAnswered}>
                                        {answer}
                                    </button>
                                ))}
                            </div>
